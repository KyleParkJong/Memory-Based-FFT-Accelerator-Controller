...
int main()
{
	...
	for (ii = 0; ii < N*FFT_ROUNDS; ii+=1) {
		...
		ref_float = 64 * ((float)ref_short/ pow(2,NUM_FRAC_BITS));

		out_FFT_hw.img = 64 * ((float)tmp_short /pow(2,NUM_FRAC_BITS));
		...
		out_FFT_hw.real = 64 * (((float)tmp_short) /pow(2,NUM_FRAC_BITS));
